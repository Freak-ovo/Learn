...
/**
  ...
  * 
  * 当接收到完整数据包时：
  *   1. 在数据末尾添加'\0'使其成为C字符串
  *   2. 设置Serial_RxFlag接收完成标志
  * 
  * 注意：使用静态变量保持状态，支持多次中断调用
  */
void USART1_IRQHandler(void)
{
    static uint8_t RxState = 0;    // 接收状态机状态 (0:等待起始符 1:接收数据 2:等待结束符)
    static uint8_t pRxPackt = 0;   // 数据包缓冲区索引（当前写入位置）

    /* 检查USART1接收中断标志（RXNE = 接收寄存器非空） */
    if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  
    {
        ...
        else if (RxState == 1)  // 状态1：接收数据内容
        {
            ...
            {
                // 将数据存入接收缓冲区
                Serial_RxPack[pRxPackt] = RxData;
                pRxPackt++;  // 指针位置递增
                
                // 注意：此处应添加缓冲区溢出保护
                // 例如：if(pRxPackt >= RX_BUFFER_SIZE) RxState=0;
            }
        }
        else if (RxState == 2)  // 状态2：等待数据包结束符
        {
            if (RxData == '\n')  // 检测到换行符（完整结束）
            {
                RxState = 0;  // 重置状态机
                Serial_RxPack[pRxPackt] = '\0';  // 添加字符串结束符
                Serial_RxFlag = 1;  // 设置接收完成标志
            }
            else  // 非预期字符处理
            {
                // 可在此添加错误处理
                RxState = 0;  // 重置状态机（丢弃无效数据）
            }
        }
