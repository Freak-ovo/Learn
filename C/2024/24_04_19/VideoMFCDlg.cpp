...
BEGIN_MESSAGE_MAP(CVideoMFCDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EdtConfidence2, &CVideoMFCDlg::OnEnChangeEdtconfidence2)
END_MESSAGE_MAP()
...
BOOL CVideoMFCDlg::OnInitDialog()
{
	...
	pEdtname = (CEdit*) GetDlgItem(IDC_EdtConfidence2);
	pEdtnumber = (CEdit*) GetDlgItem(IDC_EdtConfidence3);
	pTip = (CEdit*) GetDlgItem(IDC_Tip);
	pEdtConfidence->SetWindowText("70");
	pEdtConfidence->GetWindowText(strConfidence);
	pEdtname->GetWindowText(str_name);
	pEdtnumber->GetWindowText(str_number);
...
void CVideoMFCDlg::OnBnClickedPhotograph()
{
	...
	try
	{
		path = (const char *)str_name;	
		number = (const char *)str_number;	
	}
	catch(cv::Exception &e)
	{
		MessageBox("姓名和学号输入有误！");
		return;
	}
	string faceImgName = "..//einfacedata//trainingdata//"+path+"+"+number+".jpg";
	imwrite(faceImgName,img);
 
	//pTip->GetWindowText(strTip);
	CString tipPhoto = strTip + "\r\n拍照成功！已存为" + ("/einfacedata/trainingdata/"+path+".jpg").c_str();
	pTip->SetWindowText( tipPhoto );
	//MessageBox("OK");
}
...
void CVideoMFCDlg::OnEnChangeEdtconfidence2()
{
