...
target_thetas = np.array([0, 0])  # 目标角度（0°，同角度）
...
for idx in SI_indices:
    a = steering_vector(N, d, fc, rho_range[idx], 0)
...
for i, tr in enumerate(target_rhos):
    ...
    theta_idx = np.argmin(np.abs(theta_range_new - 0))
    gain_tr = beam_gains_2d[tr_idx, theta_idx]
    print(f"目标{i+1} ({tr}m, 0°) 波束增益: {10*np.log10(gain_tr/1e-3):.1f} dBm")
print("="*50)